...
nnue.nnue_init(b"nn.bin")

###############################################################################
# Piece-Square tables. Tune these to change sunfish's behaviour
###############################################################################

piece = { 'P': 100, 'N': 320, 'B': 330, 'R': 525, 'Q': 929, 'K': 60000 }
pnames = "KQRBNPkqrbnp"
pst = {
    'P': (   0,   0,   0,   0,   0,   0,   0,   0,
...
W_SAFE = [-1, -7, -8, -9, -15]
B_SAFE = [-1, +7, +8, +9, +15]
DEFFENDERS = {"p": -2, "r": -4, "n": -5, "b": -4, "q": -1, "k": 15,
              "P": 2, "R": 4, "N": 5, "B": 4, "Q": 1, "K": -15,
              ".": 0}
...
class Searcher:
    ...
    def bound(self, pos, gamma, depth, root=True):
        ...
        if pos.score <= -MATE_LOWER:
            return -MATE_UPPER
        if DRAW_TEST:
            if not root and self.history.count(pos.board) >= 2:# and self.history[-6:len(self.history)].count(pos.board) > 2:
                return 0
